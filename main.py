...
async def create_auth_code(user_id: int) -> str:
    code = generate_code()
    doc_ref = db.collection("auth_codes").document(code)
    doc_ref.set({
...
async def login_command(message: types.Message):
    ...
    if await is_in_blacklist(user_id):
        ...
        return
    code = await create_auth_code(user_id)
    await message.answer(
        f"🔐 <b>Ваш код для входа на сайт:</b>\n\n<code>{code}</code>\n\n"
        "Введите этот код на сайте, чтобы авторизоваться.\n"
...
async def admin_actions(callback: types.CallbackQuery):
    ...
    if action == "accept":
        ...
        await update_application_status(target_user_id, 'rejected')
        try:
            await bot.send_message(target_user_id, f"🔇 <b>Вы получили ограничение на отправку сообщений (Мут) на 1 час.</b>", parse_mode="HTML")
        except:
            pass
        ...
        await update_application_status(target_user_id, 'rejected')
        try:
            await bot.send_message(target_user_id, "⛔ <b>Вы внесены в черный список бота.</b>", parse_mode="HTML")
        except:
            pass
...
async def admin_menu_actions(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "admin_applications":
        ...
        if not msgs:
            ...
            for m in msgs[:10]:
                text += f"👤 {m[3]} (<code>{m[1]}</code>): {m[4][:30]}... [{m[7]}]\n"
            await callback.message.answer(text, parse_mode="HTML")
        await callback.answer()
